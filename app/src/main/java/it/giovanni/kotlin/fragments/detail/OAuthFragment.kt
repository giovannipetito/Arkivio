...
class OAuthFragment : DetailFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        Logger.getInstance().setExternalLogger { tag, message, additionalMessage, level, errorCode ->
            ...
            Log.d(TAG, "$tag $message $additionalMessage $level $errorCode")
        }
    ...
    private fun callGraphAPI() {
        ...
        val request = object : JsonObjectRequest(Method.GET, MSGRAPH_URL, parameters, Response.Listener { response ->
            ...
            override fun getHeaders(): Map<String, String> {
                ...
                Log.d(TAG, "1) authResult!!.accessToken: " + authResult!!.accessToken)
                return headers
            }
        }
    ...
    */
        ...
        return object : AuthenticationCallback<AuthenticationResult> {
            override fun onSuccess(authenticationResult: AuthenticationResult?) {
                if (authenticationResult == null || TextUtils.isEmpty(authenticationResult.accessToken)
                    ...
                    Log.d(TAG, "2) authenticationResult?.accessToken: " + authenticationResult?.accessToken)
    ...
    private fun logHttpErrors(authException: AuthenticationException) {
        ...
        Log.d(TAG, "HTTP Response code: " + authException.serviceStatusCode)
        if (httpResponseCode < 200 || httpResponseCode > 300) {
            ...
            if (headers != null) {
                ...
                Log.e(TAG, "HTTP Response headers: $sb")
            }
        }
    }
    ...
    private fun getAuthInteractiveCallback(): AuthenticationCallback<AuthenticationResult> {
        return object : AuthenticationCallback<AuthenticationResult> {
            override fun onSuccess(authenticationResult: AuthenticationResult?) {
                ...
                Log.d(TAG, "3) authenticationResult.accessToken: " + authenticationResult.accessToken)
                Log.d(TAG, "4) authenticationResult.idToken: " + authenticationResult.idToken)
            ...
            override fun onError(exception: Exception) {
                ...
                if (exception is AuthenticationException) {
                    when (exception.code) {
                        ADALError.AUTH_FAILED_CANCELLED -> Log.e(TAG, "The user cancelled the authorization request")
